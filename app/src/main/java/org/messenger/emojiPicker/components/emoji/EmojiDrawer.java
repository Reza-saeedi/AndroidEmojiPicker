...
public class EmojiDrawer extends LinearLayout implements InputAwareLayout.InputView {
    ...
    public void setDataType(boolean stickerEnable, boolean emojiEnable) {
        final View v = LayoutInflater.from(getContext()).inflate(R.layout.emoji_drawer, this, true);
        initializeResources(v);
        ...
        if (stickerEnable) {
            ...
            this.models.addAll(EmojiPages.StickerPAGES);
        }
        ...
        pager.setAdapter(new EmojiPagerAdapter(getContext(),
                ...
                new EmojiPageView.EmojiSelectionListener() {
                    ...
                    public void onStickerSelected(EmojiPageModel emojiPageModel) {
                        if (listener != null) listener.onStickerSelected(emojiPageModel);
                    }
                }));

        if (recentModel!=null && recentModel.getEmoji().length == 0) {
